...
function PostsByCategory() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/posts/category/${id}`, {
      headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
    }).then(res => {
      setPosts(res.data);
      if (res.data.length > 0) setCategoryName(res.data[0].category_name);
    }).catch(console.error);
  }, [id]);
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: '800px', margin: '32px auto', padding: '0 16px' }}>
        <h2 style={{ color: '#7c3a00' }}>Category: {categoryName}</h2>
        {posts.length === 0 ? (
          <p style={{ color: '#888' }}>No posts in this category.</p>
        ) : (
          posts.map(post => (
